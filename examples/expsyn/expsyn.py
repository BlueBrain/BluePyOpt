...
def main():
    ...
    rec = ephys.recordings.CompRecording(
        name="soma.v", location=somacenter_loc, variable="v"
    )

    protocol = ephys.protocols.SweepProtocol(
        "netstim_protocol", [netstim], [rec]
    )
    ...
    max_volt_objective = ephys.objectives.SingletonObjective(
        max_volt_feature.name, max_volt_feature
    )
    ...
    responses = protocol.run(
        cell_model=cell, param_values=best_ind_dict, sim=nrn_sim
    )
