...
class Protocol(object):
    ...
    def __init__(self, stim_vec, delta_vec, f, n, prot_id=None):
        ...
        for s in stim_vec:
            assert s in valid_stim, "'{0}' is not a recognised stimulus".format(
                s
            )
...
class CalciumTrace(object):
    ...
    def materializetrace(self):
        ...
        for j in xrange(len(self.__evnt)):
            ...
            component = self.amplitude[j] * np.exp(
                -(tvec[: n - offset] / self.model["tau_ca"])
            )
...
def time_above_threshold(protocol, param):
    ...
    for i in xrange(n_events - 1):
        C_amp[i] = baseline * np.exp(
            -np.sum(np.abs(ca_event_delta[: i + 1])) / param["tau_ca"]
        )
        logging_debug("C_amp[%d] = 0.0", i)
        for j in xrange(i + 1):
            C_amp[i] += ca_event_amp[j] * np.exp(
                -np.sum(np.abs(ca_event_delta[j : i + 1])) / param["tau_ca"]
            )
            logging_debug(
                "C_amp[%d] += %s * exp(-sum(abs(deltas[%d:%d])))",
                i,
                protocol.stim_vec[j],
                j,
                i + 1,
    ...
    for i in xrange(n_events):
        if C_amp[i] > param["theta_d"]:
            ...
            t_d[i] = param["tau_ca"] * np.log(
                C_amp[i + n_events] / param["theta_d"]
            )
        else:
            t_d[i] = 0.0
    t_d_tot = np.sum(t_d)
    ...
    for i in xrange(n_events):
        if C_amp[i] > param["theta_p"]:
            ...
            t_p[i] = param["tau_ca"] * np.log(
                C_amp[i + n_events] / param["theta_p"]
            )
        else:
            t_p[i] = 0.0
    t_p_tot = np.sum(t_p)
...
def transition_prob(protocol, param=None):
    ...
    sigma_rho_sq = (
        param["sigma"] ** 2 * (alpha_p + alpha_d) / (big_gamma_p + big_gamma_d)
    )
...
def protocol_outcome(protocol, param=None):
    ...
    sg = (
        ...
        + down * (1.0 - param["beta"])
        + param["b"]
        * (up * param["beta"] + (1.0 - down) * (1.0 - param["beta"]))
