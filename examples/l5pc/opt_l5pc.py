...
def create_optimizer(args):
    ...
    opt = bluepyopt.optimisations.DEAPOptimisation(
        evaluator=evaluator, map_function=map_function, seed=seed
    )
...
The folling environment variables are considered:
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="Seed to use for optimization"
    )
    parser.add_argument(
        "--ipyparallel",
        action="store_true",
...
def main():  # pylint: disable=too-many-statements
    ...
    if args.analyse:
        ...
        if args.checkpoint is not None and os.path.isfile(args.checkpoint):
            ...
            print(
                "No checkpoint file available run optimization "
                "first with --start"
            )
        ...
        l5pc_analysis.analyse_releasecircuit_hocmodel(
            opt=opt, fig=fig_release, box=box
        )
        ...
        l5pc_analysis.plot_diversity(
            opt, args.diversity, fig_diversity, opt.evaluator.param_names
        )
