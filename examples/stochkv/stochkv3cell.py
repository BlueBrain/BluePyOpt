...
def stochkv3_hoc_filename(deterministic=False):
    ...
    return os.path.join(
        script_dir, "stochkv3cell%s.hoc" % ("_det" if deterministic else "")
    )
...
def run_stochkv3_model(deterministic=False):
    ...
    morph = ephys.morphologies.NrnFileMorphology(
        os.path.join(morph_dir, "simple.swc")
    )
    ...
    pas_mech = ephys.mechanisms.NrnMODMechanism(
        name="pas", suffix="pas", locations=[somatic_loc]
    )
    ...
    rec = ephys.recordings.CompRecording(
        name="Step.soma.v", location=soma_loc, variable="v"
    )
    ...
    hoc_string = stochkv3_cell.create_hoc(
        param_values=best_param_values, disable_banner=True
    )

    stochkv3_hoc_cell = ephys.models.HocCellModel(
        "stochkv3_hoc_cell", morphology_path=morph_dir, hoc_string=hoc_string
    )
...
def main():
    ...
    for deterministic in [True, False]:
        (
            stochkv3_responses,
            stochkv3_hoc_responses,
            different_seed_responses,
            stochkv3_hoc_string,
        ) = run_stochkv3_model(deterministic=deterministic)

        with open(
            stochkv3_hoc_filename(deterministic=deterministic), "w"
        ) as stochkv3_hoc_file:
        ...
        different_seed_voltage = different_seed_responses["Step.soma.v"][
            "voltage"
        ]
