...
def stochkv_hoc_filename(deterministic=False):
    ...
    return os.path.join(
        script_dir, "stochkvcell%s.hoc" % ("_det" if deterministic else "")
    )
...
def run_stochkv_model(deterministic=False):
    ...
    stochkv_hoc_cell = ephys.models.HocCellModel(
        "stochkv_hoc_cell", morphology_path=morph_dir, hoc_string=hoc_string
    )
...
def main():
    ...
    for deterministic in [True, False]:
        (
            stochkv_responses,
            stochkv_hoc_responses,
            different_seed_responses,
            stochkv_hoc_string,
        ) = run_stochkv_model(deterministic=deterministic)

        with open(
            stochkv_hoc_filename(deterministic=deterministic), "w"
        ) as stochkv_hoc_file:
