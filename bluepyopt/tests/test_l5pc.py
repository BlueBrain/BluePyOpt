...
neuron_sim.neuron.h.nrn_load_dll(os.path.join(L5PC_PATH, "x86_64/.libs/libnrnmech.so"))
...
class TestL5PCModel(object):
    ...
    def setup(self):
        ...
        nt.assert_is_instance(self.l5pc_cell, bluepyopt.ephys.models.CellModel)
...
class TestL5PCEvaluator(object):
    ...
    def test_eval(self):
        ...
        result = self.l5pc_evaluator.evaluate_with_dicts(param_dict=release_parameters)
        ...
        try:
            nt.assert_count_equal(
                result, expected_results["TestL5PCEvaluator.test_eval"]
            )
...
def test_exec():
    ...
    try:
        ...
        nt.assert_true("'gamma_CaDynamics_E2.somatic': 0.03229357096515606" in stdout)
    finally:
        os.chdir(old_cwd)
        output.close()
