...
STOCHKV_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../examples/stochkv")
)
...
neuron_sim.neuron.h.nrn_load_dll(
    os.path.join(STOCHKV_PATH, "x86_64/.libs/libnrnmech.so")
)
...
def test_run():
    ...
    for deterministic in [True, False]:
        ...
        expected_hoc_filename = os.path.join(
            STOCHKV_PATH, stochkvcell.stochkv_hoc_filename(deterministic=deterministic)
        )
