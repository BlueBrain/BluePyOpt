...
def test_cellmodel_namecheck():
    ...
    for name in ["3test", "", "test$", "test 3"]:
        nt.assert_raises(
            TypeError, ephys.models.CellModel, name, morph=test_morph, mechs=[]
        )
...
def test_CellModel_create_hoc():
    ...
    morph0 = ephys.morphologies.NrnFileMorphology(
        simple_morphology_path, do_replace_axon=True
    )

    cell_model = ephys.models.CellModel(
        "CellModel", morph=morph0, mechs=[], params=[]
    )
    ...
    cell_model_hoc = ephys.models.HocCellModel(
        "CellModelHOC", simple_morphology_path, hoc_string=hoc_string
    )
...
def test_CellModel_destroy():
    ...
    cell_model0 = ephys.models.CellModel(
        "CellModel_destroy", morph=morph0, mechs=[], params=[]
    )
    morph1 = ephys.morphologies.NrnFileMorphology(simple_morphology_path)
    cell_model1 = ephys.models.CellModel(
        "CellModel_destroy", morph=morph1, mechs=[], params=[]
    )
...
def test_metaparameter():
    ...
    paramB = ephys.parameters.MetaParameter(
        "ParamB", scaler, "B", bounds=[1.0, 3.0]
    )
    paramC = ephys.parameters.MetaParameter(
        "ParamC", obj=scaler, attr_name="C", value=0.003, frozen=True
    )
    ...
    cell_model = ephys.models.CellModel(
        "CellModel", morph=morph, mechs=[], params=[cm, paramA, paramB, paramC]
    )

    nt.assert_raises(Exception, cell_model.freeze, {"ParamC": 2.0})
    ...
    nt.assert_equal(
        scaler.eval_dist({"value": 1.0}, 1.0),
        "(-1 + 2.0 * math.exp(1 * 0.003)) * 1"
    )

    nt.assert_almost_equal(
        scaler.scale(1.0, cell_model.icell.apic[0](0.5), sim=sim),
        1.045510068328892,
    )
    ...
    nt.assert_true(
        'distribute_distance(CellRef.all, "cm", "(-1 + 2.0 * '
