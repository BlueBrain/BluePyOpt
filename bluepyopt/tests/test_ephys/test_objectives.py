...
def test_EFeatureObjective():
    ...
    nt.assert_almost_equal(
        e_obj.calculate_feature_scores(responses), [abs(efeature_value - mean)]
    )
...
def test_SingletonObjective():
    ...
    s_obj = ephys.objectives.SingletonObjective("singleton", feature=efeature)
    ...
    nt.assert_almost_equal(
        s_obj.calculate_score(responses), abs(efeature_value - mean)
    )
...
def test_MaxObjective():
    ...
    efeature1 = ephys.efeatures.eFELFeature(
        name="test_eFELFeature",
        efel_feature_name="voltage_base",
        recording_names=recording_names,
        stim_start=700,
    ...
    max_obj = ephys.objectives.MaxObjective(
        "max", features=[efeature1, efeature2]
    )
...
def test_WeightedSumObjective():
    ...
    w_obj = ephys.objectives.WeightedSumObjective(
        "weighted", features=[efeature], weights=[weight]
    )
    ...
    nt.assert_almost_equal(
        w_obj.calculate_score(responses), abs(efeature_value - mean) * weight
    )
