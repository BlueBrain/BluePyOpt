...
class TestNumbaModels(object):
    def setup(self):
        ...
        trans_dict = collections.OrderedDict(
            [
                (k, [])
                for k in ["C", "k", "vr", "vt", "vPeak", "a", "b", "c", "d", "celltype"]
            ]
        )
        ...
        reduced_cells = collections.OrderedDict(
            [(k, []) for k in ["RS", "IB", "TC", "LTS", "RTN", "FS", "CH"]]
        )
    ...
    def test_models(self):
        ...
        for cellmodel in cellmodels:
            ...
            try:
                assert float(int(np.round(vm.times[-1], 0))) == float(
                    ALLEN_DELAY
                ) + float(ALLEN_DURATION)
            except:
                print(
                    float(int(np.round(vm.times[-1], 0)))
                    == float(ALLEN_DELAY) + float(ALLEN_DURATION)
                )
        cellmodel = "IZHI"
        ...
        model = model_classes.ADEXPModel()
        model.set_attrs({"b": self.reduced_cells["RS"]["b"]})
        nt.assert_equal(model.attrs["b"], self.reduced_cells["RS"]["b"])
        for i, amp in enumerate(IinRange):
            model = model_classes.IzhiModel()
            model.set_attrs(self.reduced_cells["RS"])
            nt.assert_equal(model.attrs["a"], self.reduced_cells["RS"]["a"])
        ...
        for i, amp in enumerate(IinRange):
            ...
            model.set_attrs(self.reduced_cells["IB"])
            nt.assert_equal(model.attrs["a"], self.reduced_cells["IB"]["a"])
